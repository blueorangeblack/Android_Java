...
public class MainActivity extends AppCompatActivity {
    ...
    //재생할 위치의 노래의 인덱스를 받아서 재생기에 노래를 설정하는 메소드
        ...
        new Thread(){
            ...
            public void run(){
                try{
                    ...
                    //옵션 설정 - 파라미터 전송방법, 시간, 캐시사용여부, 파라미터 전송처리
                    con.setConnectTimeout(30000); //거의 무조건 설정하는 것이 좋음. 일정한 시간동안 다운로드 안받으면 중지됨
                    ...
                    //문자열을 제대로 다운받았는지 확인
                    Snackbar.make(getWindow().getDecorView().getRootView(),
                            "다운받은 문자열"+sb.toString(), Snackbar.LENGTH_SHORT).show();
                    ...
                    //Log.e("배열", songList.toString()); 배열은 이렇게 하면 안돼
                    ...
                    player.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
                        ...
                        public void onCompletion(MediaPlayer mp) {
                            //마지막 노래면 첫번째 노래로 돌아가고 그렇지 않으면 현재 인덱스 +1
                    ...
                    //player가 노래를 재생하다가 에러가 발생한 경우
                    //(MediaPlayer의 onErrorListener가 처리)
                     player.setOnErrorListener(new MediaPlayer.OnErrorListener() {
                         @Override
                         public boolean onError(MediaPlayer mp, int what, int extra) {
                             Snackbar.make(getWindow().getDecorView().getRootView(),
                                     "재생 중 에러 발생", Snackbar.LENGTH_SHORT)
                                     .show();
                             return false;
                         }
                     });
                    ...
                    //Seekbar의 thumb을 사용자가 움직였을 때 처리하는 이벤트 핸들러
                    progress.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
                        //Seekbar의 thumb을 움직이고 난 후 호출되는 메소드
                        @Override
                        public void onProgressChanged(
                                SeekBar seekBar, int progress, boolean fromUser) {
                            if (fromUser == true){ //사용자에 의해 움직이면
